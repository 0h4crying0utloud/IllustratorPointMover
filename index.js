 ...
 * 
 * @Testing
 * - node index.js  (will use mock data)
 * 
 * @Running
 * - CTRL + SHIFT + P > chose Illustrator from menu
 * 
 */
...
function moveCurvePoints(sides) {
    ...
    var xoffset, yoffset;
    for(var i=0; i<sides.length; i++) { // for each side

        sidePoints = sides[i];
        for(var j=0; j<sidePoints.length; j++) { // each side has four point goups

            try {
                pointGroup = sidePoints[j];
                xoffset = stdRandRange();
                yoffset = stdRandRange();
    
                firstPoint = sidePoints[j][0];
                first_anchor = [firstPoint.ref.anchor[0] + xoffset, firstPoint.ref.anchor[1] + yoffset];
                first_leftDirection = [firstPoint.ref.leftDirection[0] + xoffset, firstPoint.ref.leftDirection[1] + yoffset];
                first_rightDirection = [firstPoint.ref.rightDirection[0] + xoffset, firstPoint.ref.rightDirection[1] + yoffset];
                ...
                if (overlappedPoint) {
                    overlappedPoint.ref.anchor =  first_anchor;
                    overlappedPoint.ref.leftDirection = first_rightDirection;
                    overlappedPoint.ref.rightDirection = first_leftDirection;
                }
